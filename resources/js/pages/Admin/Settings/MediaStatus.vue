...
const editingStatus = ref<MediaStatus | null>(null);
...
const openEditModal = (status: MediaStatus) => {
...
const saveStatus = () => {
  const url = editingStatus.value
    ? route('admin.settings.media-status.update', editingStatus.value.id)
    : route('admin.settings.media-status.store');
...
const deleteStatus = (status: MediaStatus) => {
  if (confirm(`Tem certeza que deseja excluir o status "${status.title}"?`)) {
    router.delete(route('admin.settings.media-status.destroy', status.id));
  }
};
</script>
...
<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl text-gray-700 font-bold">Status da Mídia</h1>
    ...
    <div class="grid gap-4">
      <Card v-for="status in props.mediaStatuses" :key="status.id">
        <CardHeader>
      ...
      <div v-if="!props.mediaStatuses || props.mediaStatuses.length === 0" class="text-center py-8 text-gray-500">
        Nenhum status de mídia cadastrado. Clique em "Novo Status" para começar.
      </div>
    </div>
    ...
    <Dialog v-model:open="showModal">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {{ editingStatus ? 'Editar Status da Mídia' : 'Novo Status da Mídia' }}
          </DialogTitle>
        </DialogHeader>
        ...
        <form @submit.prevent="saveStatus" class="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Descrição do status da mídia..."
